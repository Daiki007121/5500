...
export const getUserInfo = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: "Server Error trying to get USER INFO BY ID"});
    }
};
...
export const deleteUser = async (req,res) => {
    try {
        ...
        return res.status(200).json({message: `User deleted with name: ${deletedUser.name}`});

    } catch (err) {
        console.error(`BAD DELETE REQUEST USER!! \n Error: ${err}`);
        return res.status(500).json({ message: "DELETE request user failed. Try again"})
    }
};